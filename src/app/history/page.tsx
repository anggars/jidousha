                  ...
                  <>
                    <Card className="bg-card border-border animate-scale-in">
                      <CardHeader className="border-b border-border pb-4">
                        <CardTitle className="text-lg font-bold text-foreground">{t('topPlayers')}</CardTitle>
                      </CardHeader>
                      <CardContent className="p-0">
                        <div className="divide-y divide-border">
                          {leaderboard.map((user, idx) => (
                            <div key={user.name} className="flex items-center justify-between p-4 hover:bg-secondary/50 transition-colors">
                              <div className="flex items-center gap-4">
                                <span className="text-sm font-bold text-muted-foreground w-4 text-right">#{idx + 1}</span>
                                ...
                                <div className="flex flex-col">
                                  <span className="text-sm font-bold text-foreground">
                                    ...
                                    {user.name === currentProfile.name && (
                                      <span className="ml-2 text-[9px] bg-primary/10 text-primary border border-primary/20 px-1.5 py-0.5 rounded-full font-normal">
                                        {t('you')}
                                      </span>
                                    )}
                                  </span>
                                  <span className="text-xs text-muted-foreground">{user.totalExams} {t('totalExams').toLowerCase()}</span>
                                </div>
                              </div>
                              <div className="flex items-center gap-4">
                                <div className="flex flex-col items-end">
                                  <span className="text-sm font-bold text-foreground">{user.bestScore} {t('correct').toLowerCase()}</span>
                                  <span className="text-[10px] text-muted-foreground uppercase">{t('highestScore')}</span>
                                </div>
                                <div className="w-12 text-right">
                                  <span className="text-sm font-bold text-primary">{user.averageScore}%</span>
                                </div>
                              </div>
                            </div>
                          ))}
                ...
                {globalHistory.length === 0 ? (
                  <Card className="bg-card border-border py-12 text-center">
                    <CardContent className="space-y-2">
                      ...
                      <p className="text-sm text-muted-foreground">{t('noTimelineData')}</p>
                    </CardContent>
                  </Card>
                ) : (
                    ...
                    {globalHistory.map((item, index) => {
                      ...
                      return (
                        <div key={item.id || index} className="relative pl-6 animate-scale-in">
                          {/* Timeline Marker */}
                          <div className="absolute -left-1.5 top-1.5 w-3 h-3 bg-card border border-border rounded-full flex items-center justify-center">
