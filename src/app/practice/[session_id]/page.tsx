...
export function PracticeQuizContent({ sessionId }: { sessionId?: string }) {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      ...
      // Always use local questions for now since DB only has 5 seeds
                  ...
                  <div key={q.id} className="pt-8 first:pt-0 space-y-4">
                    <div className="flex justify-between items-start gap-4">
                      <div className="space-y-1">
                        ...
                        <p className="text-sm text-muted-foreground italic font-normal leading-relaxed">{qTranslated}</p>
                      </div>
